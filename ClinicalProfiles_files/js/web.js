		...
		if (childNodes[c].tagName == 'ownedElement') {
					...
					if (members[p].getAttribute('isRelationship') == 'true') {
						...
						if (parentUL.firstChild && parentUL.firstChild.elementName == 'Relations')
							...
							var relationLI = addNode(parentUL, 'Relations', 'javascript:void(0);',
								'ClinicalProfiles_files/icon_Relationship_642538759.jpg', 'true');
			...
			for ( var m = 0; m < members.length; m++) {
				if (members[m].nodeType == 1) {
					...
					var childNode = addChildNode(parentUL, name, members[m], icon);
...
function findNodeForSelected(nodeId, selectInTree) {
	...
	if (tree.root && tree.root.index) {
		...
		if (content.model) {
			...
			findNodeByAttribute(dataModel, 'refid', nodeId, searchResults);
...
function renderBrowser(responseXML) {
	...
	if (responseXML) {
		...
		*/
		...
		var dataModels = getModels(responseXML.firstChild);
 ...
 */
function findNodeByAttribute(node, attr, value, searchResults) {
	// if (node.tagName == 'ownedDiagram')
	// return;
	if (node.nodeType == 1) {
	...
	if (node.hasChildNodes()) {
		var childNodes = node.childNodes;
		for ( var i = 0; i < childNodes.length; i++)
			findNodeByAttribute(childNodes[i], attr, value, searchResults);
	}
}
